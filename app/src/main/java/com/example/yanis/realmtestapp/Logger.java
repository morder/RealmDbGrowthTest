...
public final class Logger {
    ...
    public void showDbStats(@Nullable String prepend) {
        if (isEnabled) {
            StringBuilder stringBuilder = new StringBuilder((prepend != null ? prepend + ": " : "") + "DB files size: ");
            String root = "/data/data/" + MyApplication.getInstance().getPackageName();
            int i = 0;
            for (RealmCreator.Db db : RealmCreator.Db.values()) {
                File file = new File(root + "/files/" + db.name() + ".realm");
                if (i > 0) {
                    stringBuilder.append(", ");
                }
                NumberFormat formatter = NumberFormat.getInstance();
                String result = formatter.format(file.length());
                stringBuilder.append(db.name()).append(": ").append(result);

                i++;
            }

            Log.e(className, stringBuilder.toString());
        }
    }
}
